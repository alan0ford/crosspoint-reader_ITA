...
Se lo hai già acquistato - *magari ti sei fatto incantare da qualche video recensione* -  li avrai scoperti da te, per quelli invece che sono in dubbio, spero possa essere utile elencare quelli che a mio parere ho riscontrato:
 ...
 - assenza di un ebook store affiliato (in sostanza nessuna gestione dei DRM);
 - limitato supporto dei formati (epub e txt) e non completa gestione della formattazione degli epub;
...
Inoltre se connessi alla medesima WiFi (o all'Hotspot che crea l'X4) è possibile navigare fra le cartelle direttamente via browser:
http://crosspoint.local/

Per la creazione dei wallpaper (se quelli proposti da Send to X4 non piacessero), consiglio la pagina web:
